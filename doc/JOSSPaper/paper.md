...
<path/filename> = arqfit: name of the file to save fitness
        <value> =     nu: number of unknowns (dim. of the trial vector X)
  <value of X1>
        .
        .
        .
...
where $F$ is the differentiation parameter. Then, the mutant individual and $\boldsymbol{x}_i$ cross over, producing the trial individual $\boldsymbol{u}$. The components $u_j$ ($1 \le j \le D$) of $\boldsymbol{u}$ are given by 
$$u_{j}=\begin{cases} 
...
Finally, Tab. 3 presents the mean computational time per generation $\tau$ for different number of threads $N_t$. As can be seen, $\tau_{N_t}$ is approximately proportional to $\tau_1/N_t$.
